...
### Click Evt
1.Random Ball 
- Find mouse position 마우스 위치 찾기
- Click Event 클릭이벤트 : 요소 생성 
- Random color 랜덤으로 색상 설정
- Random size 랜덤으로 사이즈 설정
- Sound source 공을 누르는 사운드 효과
...
2.Bomb Aircap 
- Render same element with a component 컴포넌트로 반복 랜딩
- Click Event 클릭이벤트 : 이미지 변경 
- Sound source 에어캡을 터트리는 사운드 효과
...
3.Ball
- Render same element with component 컴포넌트로 ball 랜딩
- Click Event 클릭 이벤트 : 스타일 변경 
- accelerate 공의 가속도 설정
...
### MouseOver Evt
1.Dori Dori 
- Render same element with component 컴포넌트로 글자 랜딩
- MouseOver Event 마우스 오버 이벤트 : 스타일 변경 
...
- Mouseover Event 마우스 오버에 따른 요소 Rotate
...
- Keyup Event 키업 이벤트 : 스타일 변경 (up,down,right,left 위치 조정) 
- DarkMode 다크모드 설정
- Connect Size with Opacity : 컴포넌트에 전달되는 사이즈에 따라 다른 불투명도로 설정 (가오리)
- Sound source : 사용자의 원함에 따라 배경음악 turn on/off
